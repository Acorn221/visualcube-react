...
const faceIdentity: FaceRotation = (stickerNumber: number, _cubeSize: number) =>
  stickerNumber;
    ...
    for (let i = 0; i < this.#cubeSize; i++) {
      for (let r = 0; r < range; r++) {
        ...
        for (let j = 0; j < faceOrder.length; j++) {
          ...
          const valueIndex =
            AxisOrientation[axis][face](stickerIndex + 1, this.#cubeSize) - 1;
          const nextFaceValueIndex =
            AxisOrientation[axis][nextFace](stickerIndex + 1, this.#cubeSize) -
            1;
          this.faces[face][valueIndex] =
            originalValues[(double ? j + 2 : j + 1) % originalValues.length][
              nextFaceValueIndex
            ];
        }
      }
